...
import no.nav.security.token.support.spring.validation.interceptor.BearerTokenExchangeFilterFunction;
...
public class EnableJwtTokenValidationConfiguration implements WebMvcConfigurer, EnvironmentAware, ImportAware {
    ...
    @Bean
    public ClientHttpRequestInterceptor bearerTokenClientHttpRequestInterceptor(
            TokenValidationContextHolder tokenValidationContextHolder) {
    ...
    @Bean
    public ExchangeFilterFunction bearerTokenExchangeFilterFunction(
            TokenValidationContextHolder tokenValidationContextHolder) {
        logger.info("creating bean for bearerTokenExchangeFilterFunction");
        return new BearerTokenExchangeFilterFunction(tokenValidationContextHolder);
    }
    ...
    public JwtTokenHandlerInterceptor getControllerInterceptor() {
        ...
        return new JwtTokenHandlerInterceptor(enableOIDCTokenValidation,
                new SpringJwtTokenAnnotationHandler(oidcRequestContextHolder()));
    }
