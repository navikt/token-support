...
public class JwtTokenHandlerInterceptor implements HandlerInterceptor {
    ...
    public JwtTokenHandlerInterceptor(AnnotationAttributes enableOIDCTokenValidation,
                                      TokenContextHolder contextHolder) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        TokenValidationContext validationContext = contextHolder
        ...
        if (handler instanceof HandlerMethod) {
            ...
            throw new JwtTokenUnauthorizedException("Server misconfigured - controller/method ["
    ...
    protected boolean handleProtectedAnnotation(TokenValidationContext validationContext) {
        if (validationContext.hasValidToken()) {
            return true;
        }
        logger.debug("no token found in validation context");
        throw new JwtTokenUnauthorizedException("Authorization token required");
    }

    protected boolean handleProtectedWithClaimsAnnotation(TokenValidationContext validationContext,
        ...
        if (StringUtils.isNotBlank(issuer)) {
            ...
            if (tokenClaims == null) {
                logger.trace(String.format(
                        ...
                        issuer));
                throw new JwtTokenUnauthorizedException("Authorization token not authorized");
            }
