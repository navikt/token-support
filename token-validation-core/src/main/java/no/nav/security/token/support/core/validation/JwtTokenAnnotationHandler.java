...
public class JwtTokenAnnotationHandler {
    ...
    public boolean assertValidAnnotation(Method method) throws AnnotationRequiredException {
        Annotation annotation = getAnnotation(method,
                Arrays.asList(ProtectedWithClaims.class, Protected.class, Unprotected.class));
        if (annotation == null) {
    ...
    private boolean assertValidAnnotation(Annotation annotation) {
        if (annotation instanceof Unprotected) {
            ...
            log.debug("annotation is of type={}, do token validation and claim checking.",
                    ProtectedWithClaims.class.getSimpleName());
            ...
            log.debug(
                    "annotation is unknown,  type={}, no token validation performed. but possible bug so throw exception",
    ...
    protected boolean handleProtectedWithClaims(String issuer, String[] requiredClaims, boolean combineWithOr) {
        if (Objects.nonNull(issuer) && issuer.length() > 0) {
            ...
            if (!containsRequiredClaims(jwtToken, combineWithOr, requiredClaims)) {
                throw new JwtTokenInvalidClaimException(
                        "required claims not present in token." + Arrays.asList(requiredClaims));
            }
        }
        return true;
    }
